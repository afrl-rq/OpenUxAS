...
package vectors_cross_2d with SPARK_Mode is
  ...
  function are_vectors_collinear(v1: vector_2d;
   ...
   function are_points_collinear(p1, p2, p3: point_2d) return Boolean is
     (are_vectors_collinear(vector_from_point_to_point(p1, p2),
      vector_from_point_to_point(p1, p3)))
     with
       Pre => can_subtract(p2, p1) and then can_subtract(p3, p1) and then
     vector_2d_constraint(p2.x - p1.x, p2.y - p1.y) and then
     vector_2d_constraint(p3.x - p1.x, p3.y - p1.y);
