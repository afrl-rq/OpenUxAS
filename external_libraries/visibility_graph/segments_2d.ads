...
package segments_2d with SPARK_Mode is
  ...
  function reverse_segment(s: in segment_2d) return segment_2d is
   ...
   function is_collinear(s1, s2: segment_2d) return Boolean is
     (are_points_collinear(s1.p1, s1.p2, s2.p1) and
          are_points_collinear(s1.p1, s1.p2, s2.p2));
  ...
  function segments_share_an_endpoint(s1, s2: segment_2d) return Boolean is
   ...
   function segment_midpoint(s: segment_2d) return point_2d is
     (x => 0.5 * (s.p1.x + s.p2.x), y => 0.5 * (s.p1.y + s.p2.y));
