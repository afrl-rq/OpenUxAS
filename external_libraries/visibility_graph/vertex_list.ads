...
package vertex_list with SPARK_Mode is
  ...
  is
   ...
   function in_vertex_seq(f: bounded_vertex_list; p: point_2d) return Boolean is
     (for some i in 0 .. f.num_vertices - 1 =>
         f.vertices(i) = p);
   ...
   --                   ENDIF
   --     #);
   function append_to_uniq_vertex_seq(s: uniq_vertex_list; p: point_2d) return uniq_vertex_list is
     ((num_vertices => s.num_vertices + 1,
       vertices => (for i in 0 .. MAX_NUM_VERTICES-1 => 
                      (if i = s.num_vertices then p else s.vertices(i)))));
