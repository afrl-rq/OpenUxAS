...
package body vertex_injection with SPARK_Mode is
  ...
  begin
    ...
    -- PA #18
    -- This can be proved with SPARK Pro 24.0w (20230509) and with the following command:
    --   gnatprove -P%PP -j0 %X --output=oneline --ide-progress-bar --level=4 --prover=cvc5,z3,altergo,colibri,cvc4
    pragma Assume(for all j in 0 .. B.num_vertices-1 =>
    ...
    for idx in 0 .. B.num_vertices-1 loop
      ...
      -- LI #22.9
      -- Follows from LI #22.2
      -- Can prove with CVC4 and a very long time out of 2400 seconds
--      pragma Loop_Invariant(for all i in 0 .. retval.num_vertices-1 =>
      pragma Assume(for all i in 0 .. retval.num_vertices-1 =>
                              can_subtract(retval.vertices(i), e));
      -- PA #22.1
      ...
      --  is justified by
      --    Using expansion of its function can_subtract(Vect2, weak_segment_2d) and showing equality
      --  with
      --    The prior Assume
      --  end
      pragma Assume(for all i in 0 .. retval.num_vertices-1 =>
